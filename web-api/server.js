...
//GET-POST-PUT-DELETE
app.get("/", (req, res) => {
    return res.send("Hello Express App-2") // res.send ile string dönebiliriz
})
app.get("/image", (req, res) => {
    //koala.jpg dosyasını döndürmek için
    return res.sendFile(__dirname + "/koala.jpg")
})

app.get('/products', (req, res) => {
    let query = req.query;

    //query string ile limit parametresi gelirse(!) onun istediği sayıda ürünü return ederim
    if (query.limit != "" && query.limit != undefined) {
        ...
        return res.json(products) // res.json ile json dönebiliriz
...
app.delete("/products/:id", (req, res) => {
    ...
    let id = req.params.id;
    if(isNaN(id)){
        return res.status(500).send("Id is NAN! ID is not numeric!")
    }
    products = products.filter(q => q.id != id); //db den bir silme operasyonu
    return res.send("Ok. Product is deleted!")
})
...
//npm install package_name // bu şekilde mevcut projenize paket ekleyebilirsiniz
//npm install -g package_name // bu şekilde global paket ekleyebilirsiniz
//npm uninstall package_name // bu şekilde paket silebilirsiniz
//npm update package_name // bu şekilde paket güncelleyebilirsiniz
//npm list // bu şekilde yüklü paketleri görebilirsiniz
//npm list -g // bu şekilde global yüklü paketleri görebilirsiniz
//npm install <package_name> -dev // bu şekilde sadece development ortamında kullanılacak paketleri yükleyebilirsiniz
